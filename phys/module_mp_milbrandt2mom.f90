...
SUBROUTINE sedi_1D(QX1d,NX1d,cat,DE1d,iDE1d,iDP1d,gamfact1d,epsQ,epsN,dmx,VxMax,DxMax,    &
   ...
   DO nnn= 1,npassx
      ...
      do k= kbot,ktop,kdir
         ...
         if (QxPresent) then
            ...
            if (cat==5) then
               tmp1 = (icmx*QX1d(k)/NX1d(k))**thrd   
               tmp2 = min(50., 0.1*(1000.*tmp1))     
 ...
 SUBROUTINE mp_milbrandt2mom_main(WZ,T,Q,QC,QR,QI,QN,QG,QH,NC,NR,NY,NN,NG,NH,PS,          &
  ...
  real, parameter :: CHLC     =.2501e+7           
  real, parameter :: CHLF     =.334e+6            
  real, parameter :: CPD      =.100546e+4         
  real, parameter :: CPV      =.186946e+4         
  real, parameter :: RGASD    =.28705e+3          
  real, parameter :: RGASV    =.46151e+3          
  real, parameter :: EPS1     =.62194800221014    
  real, parameter :: DELTA    =.6077686814144     
  real, parameter :: CAPPA    =.28549121795       
  real, parameter :: GRAV     =.980616e+1         
  ...
  if ( T(i,k)<173. .or. T(i,k)>323.) then                            
     print*, '** STOPPING IN MICROPHYSICS: (Part 2, end) **'         
     print*, '** i,k,T [K]: ',i,k,T(i,k)                             
     stop                                                            
  endif                                                              
  ...
  if (DEBUG_ON) call check_values(Q,T,QC,QR,QI,QN,QG,QH,NC,NR,NY,NN,NG,NH,epsQ,epsN,.false.,DEBUG_abort,700)
   ...
   do i= 1,ni
      ...
      if (QN(i,nk)>epsQ .and. NN(i,nk)>epsN .and. fluxM_s(i)>0.) then
         tmp2    = 1./NN(i,nk)
         tmp1    = 1./iLAMDA_x(DE(i,nk),QN(i,nk),tmp2,iGS20,idms) 
