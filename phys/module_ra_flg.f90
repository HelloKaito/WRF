	  ...
	  pij(k) = PHYD(NK)/100.
          ...
          if (NPDE.eq.1.and.piwc(k) .gt. 1.e-7) then
		    ...
		    if (degrees(i,NK,j).lt.253.         &
                    .and.degrees(i,NK,j).gt.213.) then  
		      TEMP_IWC = exp(-7.6                 &
                   +4.*exp(-0.2443e-3*(253.-degrees(i,NK,j))**2.445)) 

		      TEMP_T = degrees(i,NK,j) - 273.  
        ...
        if (NAERO.ge.1) then
          ...
          else if (nfraca.eq.1) then 
            fraca(2)   = 0.9  
            fraca(11)  = 0.1  
            do iac = 1, mxac
              if (fraca(iac).gt.0.0) itps(iac) = 1
            end do
          else if (nfraca.eq.2) then 
          
          else if (nfraca.eq.3) then 
...
'ivd: No VERTICAL Aerosol Profile')  
...
'Flux out of range. Stop program')  
	endif 
...
300	  end do HORIZONTAL_I
   ...
   &                         fd_tot, fu_tot 
    ...
    integer :: ib, mbn, kg1_num, kg2_num, iac, ig1, ig2, i    
...
10    end do
     ...
     &             trn(i) + tgr(i)
        ...
        if (cldamnt(i).gt. 0.) then
          ...
          if (n_cld.gt.0.and.cld_group(n_cld).ne.0) then   
...
'subroutine tridag failed. Stop program')  
      endif
...
10    end do
	...
	return
	  ...
	  if ( ityp >= 1 .and. ityp <=3 )then 
	    ...
	    idtl=1
	  elseif ( ityp >=4 .and. ityp <= 8) then 
	    wlp  = wlt
	    extp = datt(1:24,ityp)
	    idtl=2
	  elseif ( ityp >=9 .and. ityp <= 18) then 
	    wlp  = wlopac
	    ...
	    idtl=3
           ...
           CALL wrf_error_fatal3("<stdin>",14447,&
'Bad Aerosol type. Stop program')  
	  endif
...
'OUT OF ALLOWABLE ARANGE. STOP program')  
...
12    end do
      ...
      h=xa(khi)-xa(klo)
      if (h.eq.0.)  CALL wrf_error_fatal3("<stdin>",14645,&
'bad xa input in splint. STOP program')  
      a=(xa(khi)-x)/h
      b=(x-xa(klo))/h
...
11    end do
      ...
      do m=1,n-1
        do i=1,n-m
          ...
          w=c(i+1)-d(i)
          den=ho-hp
          if(den.eq.0.)  CALL wrf_error_fatal3("<stdin>",14681,&
'failure in polint. STOP program')  
          den=w/den
          d(i)=hp*den
          c(i)=ho*den
12      end do
...
13    end do
     ...
     &             (10 ** (-3.49149 * (tsot - 1)) - 1)
      ...
      if (n_atau<0 .or.n_atau>mxat) CALL wrf_error_fatal3("<stdin>",14866,&
'errro in Aerosol Tau / Wavelengths. STOP program')  
      if (ifg < 0 .or. ifg > 8) CALL wrf_error_fatal3("<stdin>",14868,&
'Error in ifg: Aerosol RH% Flag. STOP program')  
      ...
      if (itps(iac).eq.1) then
        ...
        itp = iac
        if ( itp < 1 .or. itp > naer ) CALL wrf_error_fatal3("<stdin>",14878,&
'Error in itp: bad Aerosol type. STOP program')  
...
'ivd : Aerosol Profile flag. STOP program')  
        case (0)  
        ...
        case (1:2)   
          ...
          if(sump.eq.0.) CALL wrf_error_fatal3("<stdin>",15027,&
'No VERTICAL Profile OF AEROSOL TAU. STOP program')  
...
'iaform : Bad value of iaform . STOP program')  
        case(1)        
       ...
       &          a_tau(3)*wts(3)+a_tau(4)*wts(4)
          tau3(2)=a_tau(5)
          tau3y(1)=a_tau(1)      
          tau3y(2)=a_tau(1)+.6705*(a_tau(2)-a_tau(1)) 
          tau3y(3)=a_tau(2)+.4541*(a_tau(3)-a_tau(2)) 
          tau3y(4)=a_tau(3)*.5175+a_tau(4)*.4825      
     ...
     &        f    = 1.5e-7
      real t1, t2, t3, t4, t5, t6
         ...
         t5 = exp(ht/bp)
         t6 = (a+exp(ht/bp))**2
         spinhirne_sig=t1*t2/t3+t4*t5/t6   
      return
