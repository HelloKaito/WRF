     ...
     &     T_NEW(I,K,J),DT,rhocgs(I,K,J),COL,ICEMAX,NKR)
        ...
        IF (T_OLD(I,K,J).GT.213)THEN     
         ...
         DO IKL=1,NCOND
          ...
          IF (DEL2NR.EQ.0)call wrf_error_fatal3("<stdin>",947,&
     ...
     &     kts,kte,nkr)
      ...
      DO KR=1,NKR
       ...
       IF (IFALL.EQ.1)THEN
        ...
        END DO                                                 
        IF(TFALL.GE.1.E10) call wrf_error_fatal3("<stdin>",1966,&
       ...
       DOUBLE PRECISION CHECK,TPN
          CHECK=0.D0
           DO KR=1,NKR
             ...
             IF (CHECK.LT.0) call wrf_error_fatal3("<stdin>",5664,&
	...
	ELSE IF(RI.NE.0.AND.RW*1.E5.LT.RI) THEN
     ...
     &       (DEL1N_2P-RI*DEL2N_2P/PI)*DT+RI*DEL2INT_2P/PI
        ...
        ELSE
          ...
          IF (RW.LE.0) call wrf_error_fatal3("<stdin>",6575,&
"fatal error in module_mp_full_sbm (RW.LE.0), model stop")
          IF (PW.LE.0) call wrf_error_fatal3("<stdin>",6577,&
"fatal error in module_mp_full_sbm (PW.LE.0), model stop")
          IF (RI.LE.0) call wrf_error_fatal3("<stdin>",6579,&
"fatal error in module_mp_full_sbm (RI.LE.0), model stop")
          IF (PI.LE.0) call wrf_error_fatal3("<stdin>",6581,&
"fatal error in module_mp_full_sbm (PI.LE.0), model stop")
	  ...
	  ALFAPX=.5*(ALFA_DP+X_DP)
          IF (ALFAPX.LE.0) call wrf_error_fatal3("<stdin>",6586,&
...
10     continue
       do it = 1,ndiv
         ...
         if (ndiv.gt.10000) call wrf_error_fatal3("<stdin>",8142,&
...
if(pres_SI > 200000.D0.or.pres_SI < 10000.D0) call wrf_error_fatal3("<stdin>",9700,&
...
if(rhoa_SI > 2.D0) call wrf_error_fatal3("<stdin>",9747,&
...
if(FLIQFR_I(IK,I) > 0.D0) then
  ...
  if(IEVAP_ADJUST == 1 ) then
    ...
    if(FLIQFR_I(IK,I) > 1.D0) call wrf_error_fatal3("<stdin>",9895,&
"fatal error in module_mp_full_sbm (FLIQFR_I > 1), model stop 114")
   ...
   XI_MELT_SI(IK,I)=XX_MELT
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",9958,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",9960,&
"fatal error in module_mp_full_sbm , model stop 138")
  ...
  if(ISUBLIME_ADJUST == 1 ) then
    ...
    if(TS > 273.16) call wrf_error_fatal3("<stdin>",9976,&
"fatal error in module_mp_full_sbm , model stop 13655")
   ...
   (XI_SI(IK,I),rhoa_SI,Q_SUBL,FF2_SI(IK,I))
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",10005,&
"fatal error in module_mp_full_sbm (QQV < 0), model stop ")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",10007,&
...
IF(FLIQFR_I(IK,I) > FLIQFRAC_LIM) then
  ...
  if(XL_SI(IK).NE.XI_SI(IK,I)) call wrf_error_fatal3("<stdin>",10023,&
...
if(FLIQFR_S(IK) > 0.D0) then
  ...
  if(IEVAP_ADJUST == 1) then
    ...
    if(FLIQFR_S(IK) > 1.D0) call wrf_error_fatal3("<stdin>",10164,&
"fatal error in module_mp_full_sbm , model stop 905")
   ...
   (XS_SI(IK),rhoa_SI,Q_EVAP,FLIQFR_S(IK),FF3_SI(IK))
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",10214,&
"fatal error in module_mp_full_sbm , model stop 915")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",10216,&
"fatal error in module_mp_full_sbm , model stop 916")
  ...
  if(ISUBLIME_ADJUST == 1) then
    ...
    if(QQV < 0.) call wrf_error_fatal3("<stdin>",10261,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.) call wrf_error_fatal3("<stdin>",10263,&
"fatal error in module_mp_full_sbm , model stop 138")
...
IF(FLIQFR_S(IK) > FLIQFRAC_LIM) then
  ...
  if(XL_SI(IK).NE.XS_SI(IK)) call wrf_error_fatal3("<stdin>",10279,&
...
if(FLIQFR_G(IK) > 0.D0) then
  ...
  if(IEVAP_ADJUST == 1) then
    ...
    if(FLIQFR_G(IK) > 1.D0) call wrf_error_fatal3("<stdin>",10479,&
"fatal error in module_mp_full_sbm , model stop 901")
  ...
  if(ISUBLIME_ADJUST == 1)then
    ...
    if(QQV < 0.D0)  call wrf_error_fatal3("<stdin>",10556,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0)   call wrf_error_fatal3("<stdin>",10558,&
"fatal error in module_mp_full_sbm , model stop 138")
...
IF(FLIQFR_G(IK) > FLIQFRAC_LIM) then
  ...
  if(XL_SI(IK).NE.XG_SI(IK)) call wrf_error_fatal3("<stdin>",10572,&
...
if(FLIQFR_H(IK) > 0.D0) then
  ...
  if(IEVAP_ADJUST == 1) then
    ...
    if( FLIQFR_H(IK) > 1.D0) call wrf_error_fatal3("<stdin>",10809,&
"fatal error in module_mp_full_sbm , model stop 906")
   ...
   (XH_SI(IK),rhoa_SI,Q_EVAP,FLIQFR_H(IK),FF5_SI(IK))
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",10860,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",10862,&
"fatal error in module_mp_full_sbm , model stop 138")
  ...
  if(ISUBLIME_ADJUST == 1) then
    ...
    if(QQV < 0.D0) call wrf_error_fatal3("<stdin>",10908,&
"fatal error in module_mp_full_sbm , model stop 135")
    if(TIN < 150.D0) call wrf_error_fatal3("<stdin>",10910,&
"fatal error in module_mp_full_sbm , model stop 138")
...
IF(FLIQFR_H(IK) > FLIQFRAC_LIM) then
  ...
  if(XL_SI(IK).NE.XH_SI(IK)) call wrf_error_fatal3("<stdin>",10927,&
...
if(FFX < 0.D0) call wrf_error_fatal3("<stdin>",11059,&
"fatal error in module_mp_full_sbm , model stop 136")

if(total_mass_liq < 0.D0) call wrf_error_fatal3("<stdin>",11062,&
"fatal error in module_mp_full_sbm , model stop 137")
if(total_mass_ice < 0.D0) call wrf_error_fatal3("<stdin>",11064,&
"fatal error in module_mp_full_sbm , model stop 140")
if(total_mass < 0.D0) call wrf_error_fatal3("<stdin>",11066,&
...
if(FFX < 0.D0) call wrf_error_fatal3("<stdin>",11114,&
"fatal error in module_mp_full_sbm , model stop 140")

if(total_mass < 0.D0) call wrf_error_fatal3("<stdin>",11117,&
"fatal error in module_mp_full_sbm , model stop 141")
...
if(fx > 100.D0) call wrf_error_fatal3("<stdin>",11449,&
...
if(fx > 100.D0) call wrf_error_fatal3("<stdin>",11476,&
...
if(fx > 100.D0) call wrf_error_fatal3("<stdin>",11502,&
...
if(ix -1 < 1) call wrf_error_fatal3("<stdin>",11580,&
"fatal error in module_mp_full_sbm , model stop 42567")
if(ix  > ix_max) call wrf_error_fatal3("<stdin>",11582,&
...
if(chi_fra > 0.3D0 .and. pc < 75.D0) call wrf_error_fatal3("<stdin>",11597,&
"fatal error in module_mp_full_sbm , model stop 1478")
if(chi_fra > 0.6D0 .and. pc < 90.D0) call wrf_error_fatal3("<stdin>",11599,&
...
if(IKX > NKR.or.IKX < 1) call wrf_error_fatal3("<stdin>",11743,&
...
if(AR_ice < 0.D0.or.AR_ice > 1.D0) then
 call wrf_error_fatal3("<stdin>",11883,&
"fatal error in module_mp_full_sbm , model stop 9022")
endif

epsil_i = 1.D0 - AR_ice*AR_ice

if( epsil_i < 0.D0) call wrf_error_fatal3("<stdin>",11889,&
...
if(epsil_i > 0.D0) then
  ...
  if((1.D0+epsil_i)/(1.D0-epsil_i).le.0.D0) call wrf_error_fatal3("<stdin>",11903,&
...
if(pres_SI > 200000.D0.or.pres_SI < 10000.D0) call wrf_error_fatal3("<stdin>",12614,&
...
if(rhoa_SI > 2.D0) call wrf_error_fatal3("<stdin>",12658,&
...
"fatal error in module_mp_full_sbm , model stop 9999")
    ...
    if(fm_w < fm_w_soaked) call wrf_error_fatal3("<stdin>",13185,&
"fatal error in module_mp_full_sbm , model stop 9065")
    ...
    if(INEW < INK) then
      ...
      if(fmass_X(INK) < 0.D0) call wrf_error_fatal3("<stdin>",13211,&
"fatal error in module_mp_full_sbm , model stop 8020")
      if(res_mass_shed < 0.D0) call wrf_error_fatal3("<stdin>",13213,&
...
"fatal error in module_mp_full_sbm , model stop 8003")
      ...
      if(FLIQFR_X(INEW+1) < 0.D0.or.FLIQFR_X(INEW+1) > 1.D0) call wrf_error_fatal3("<stdin>",13320,&
"fatal error in module_mp_full_sbm , model stop 8004")
      if(FLIQFR_X(INK) < 0.D0.or.FLIQFR_X(INK) > 1.D0) call wrf_error_fatal3("<stdin>",13322,&
